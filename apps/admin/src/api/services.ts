...
export const patchProjectTeamMembers = async (
	projectId: string,
	...
	try {
		...
		// Normalizer: ensure times are HH:mm:ss and defaults are set
		const toHHmmss = (time?: string | null) => {
			if (!time) return '00:00:00';
			if (/^\d{1,2}:\d{2}:\d{2}$/.test(time)) return time;
			if (/^\d{1,2}:\d{2}$/.test(time)) return `${time}:00`;
			if (/^\d{1,2}$/.test(time)) return `${time.padStart(2, '0')}:00:00`;
		...
		const normalizedMembers = (teamMembers || []).map((m) => ({
			userId: (m as any).userId ?? (m as any).id ?? '',
			role: (m as any).role ?? 'Member',
			startTime: toHHmmss((m as any).startTime),
			endTime: toHHmmss((m as any).endTime),
			overlappingHoursRequired: (m as any).overlappingHoursRequired ?? 0,
			requiresReporting: (m as any).requiresReporting ?? false,
		} as CreateProjectTeamMemberInput));

		const body = { teamMembers: normalizedMembers };
		const res = await axios.patch(`/projects/${projectId}/team-members`, body, {
			headers: token ? { Authorization: `Bearer ${token}` } : undefined,
		});

		// Defensive unwrap: try common envelope shapes
		const project = (
			res?.data?.data?.project ?? res?.data?.data ?? res?.data?.project ?? res?.data
		) as Project;

		return {
			error: false,
			message: res?.data?.message || 'Team members updated successfully',
			data: project,
		};
	} catch (err: any) {
		let message = err?.response?.data?.message || err?.message || 'Failed to update team members';
		if (Array.isArray(message)) message = message.join(', ');
		return { error: true, message, data: undefined };
	}
};

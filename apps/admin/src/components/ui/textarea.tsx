...
function Textarea({ rows = 2, label, id, className, ...props }: IProps & React.ComponentProps<"textarea">) {
  const getId = React.useId()
  const inputId = id || getId
  return (
    <div className="flex flex-col gap-2">
      {label ? <Label htmlFor={inputId}
        className="text-sm font-medium text-neutral-700 dark:text-neutral-300">{label}</Label> : null}
      <textarea
        rows={rows}
        data-slot="textarea"
        className={cn(
